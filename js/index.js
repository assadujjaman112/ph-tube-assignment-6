const handleCategory = async () => {
  ...
  categories.forEach((category) => {
    ...
    div.innerHTML = `
      <button onclick = "loadCategory(${category.category_id})" class="btn">${category.category}</button>
      `;
...
const displayCategory = (catagories) => {
  if (loadedData.length > 0) {
    ...
    loadedData.forEach((videos) => {
      const div = document.createElement("div");
      div.innerHTML = `
        <div class="mb-10">
        <div class = "relative">
          <figure><img class = "w-full h-60 rounded-lg" src="${
            videos.thumbnail
          }" alt="Shoes" /></figure>
          <p class = "absolute bottom-3 right-3 bg-[#171717] text-white text-center rounded-lg w-2/5">${
            videos.others.posted_date
              ? `${
                  Math.floor(videos?.others?.posted_date / 3600) > 0
                    ? `${Math.floor(
                        videos?.others?.posted_date / 3600
                      )} hrs  ${Math.floor(
                        (videos?.others?.posted_date % 3600) / 60
                      )} min ago`
                    : `${Math.floor(
                        (videos?.others?.posted_date % 3600) / 60
                      )} min ago`
        ...
        <div class="flex items-center gap-5 mt-5 p-2">
          <div>
          ...
          <div class = "space-y-2">
              <h3 class = "text-2xl font-bold">${videos.title}</h3>
        ...
        videos.authors[0].verified
          ...
          : ""
              ...
              <p>${videos.others.views} <span> views</span></p>
          </div>
    ...
    div.innerHTML = `
          ...
          <p class = "text-3xl font-bold my-5 text-center">
